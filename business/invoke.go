...
func (c *BusinessCC) create() ([]byte, error) {
	...
	ok, err := insertRow(c.stub, TableCurrency,
		shim.Row{
			Columns: []*shim.Column{
...
func (c *BusinessCC) release() ([]byte, error) {
	...
	ok, err := replaceRow(c.stub, TableCurrency, currRow)
	if err != nil {
...
func (c *BusinessCC) assign() ([]byte, error) {
	...
	for _, v := range assign.Assigns {
		...
		if len(assetRow.Columns) == 0 {
			...
			_, err = replaceRow(c.stub, TableAssets, assetRow)
			if err != nil {
	...
	if curr.LeftCount != currRow.Columns[2].GetInt64() {
		...
		_, err = replaceRow(c.stub, TableCurrency, currRow)
		if err != nil {
